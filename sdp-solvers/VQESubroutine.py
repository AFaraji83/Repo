...
def _matrix_to_pauli_terms(matrix, atol=1e-12):
    ...
    Uses a fast recursive block-splitting method, NOT brute-force enumeration of all 4^n
    Pauli strings with a full matrix multiply each (an earlier version of this function did
    exactly that, and cost an estimated ~1.1 trillion floating-point operations for a single
    8-qubit/256x256 block -- confirmed to be the direct cause of multi-hour stalls on larger
    SDPLIB instances like arch0, since this decomposition reruns from scratch on every single
    VQE separation call).

    The method: split M into four (dim/2)x(dim/2) blocks [[A,B],[C,D]]. The Pauli coefficients
    for the *first* (most-significant) qubit position are exactly:
        I -> (A+D)/2,  X -> (B+C)/2,  Y -> i(B-C)/2,  Z -> (A-D)/2
    and each of those four (dim/2)x(dim/2) matrices is itself decomposed the same way over the
    remaining n-1 qubits, recursively, down to 1x1 (a bare coefficient). Every level of the
    recursion does O(dim^2) elementwise work in total across its branches, and dim halves each
    level, so the whole decomposition costs O(dim^2) overall -- for the same 256x256 block
    above, this brings a single call down to about a fifth of a second.

    Returns a list of (pauli_string, coefficient) tuples, where pauli_string is an n-character
    string over {I,X,Y,Z} written most-significant-qubit-first (matching Qiskit's SparsePauliOp
    string convention), e.g. "IXZ" for qubit order (q2, q1, q0). Verified to produce identical
    output (terms and coefficients) to the brute-force method it replaced, across n=1..4.
    """
    dim = matrix.shape[0]
    ...
    def recurse(block, prefix):
        ...
        if n == 1:
            coeff = block[0, 0]
            if abs(coeff) > atol:
                # Pauli decomposition of a Hermitian matrix has real coefficients (up to
                # floating point noise); drop any residual imaginary part.
                terms.append((prefix, coeff.real))
            return
        half = n // 2
        A, B = block[:half, :half], block[:half, half:]
        C, D = block[half:, :half], block[half:, half:]
        recurse((A + D) * 0.5, prefix + "I")
        recurse((B + C) * 0.5, prefix + "X")
        recurse((B - C) * 0.5j, prefix + "Y")
        recurse((A - D) * 0.5, prefix + "Z")
...
def _pauli_terms_to_spin_op(terms, num_qubits):
    ...
    Uses cudaq.SpinOperator(len(terms)) to pre-reserve capacity, then accumulates via += rather
    than the more obvious `op = op + term` in a loop. That distinction matters a lot here: with
    a fresh, un-reserved operator, each `+` was measured to cost time proportional to the
    *current* accumulated size, making the whole loop O(num_terms^2) -- confirmed directly: on
    an 8-qubit block with ~33k terms, per-term cost visibly doubled every time term count
    doubled (the textbook signature of quadratic blowup), and building the full Hamiltonian
    this way was projected to take minutes. With pre-reserved capacity, per-term cost is flat
    regardless of how many terms came before it, and the same ~33k-term Hamiltonian builds in
    about a quarter of a second -- verified to produce an identical operator (same matrix) to
    the unreserved version on a matching subset of terms.
    """
    gate_for_char = {"X": spin.x, "Y": spin.y, "Z": spin.z}
    op = cudaq.SpinOperator(len(terms))
    for pauli_string, coeff in terms:
        ...
        term = coeff * reduce(lambda a, b: a * b, factors) if factors else coeff * spin.i(0)
        op += term
    return op
